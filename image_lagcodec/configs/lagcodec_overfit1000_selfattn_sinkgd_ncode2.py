...
recon_ncode=(2,2,2). Identical training to lagcodec_overfit1000_selfattn_sinkgd.py (recon_ncode
is inference-time only -- every stage is always teacher-forced on the full sequence during
training regardless of grouping) -- this run only differs in how the periodic reconstruction
groups sibling blocks into a shared local AR chain (pairs, here, vs isolated singles there).
Ablation pair with lagcodec_overfit1000_selfattn_sinkgd_ncode4.py.
...
uv run python3 -m image_lagcodec.run_lagcodec --config image_lagcodec/configs/lagcodec_overfit1000_selfattn_sinkgd_ncode2.py
"""

run_name = "cifar_lagcodec_overfit1000_selfattn_sinkgd_ncode2"
