...
lr=5e-4, recon_ncode=(8,8,8). Identical training to lagcodec_overfit1000_selfattn_sinkgd_ncode2.py
(recon_ncode is inference-time only) -- differs only in how the periodic reconstruction groups
sibling blocks into a shared local AR chain (groups of 8, here). Ablation pair with
lagcodec_overfit1000_selfattn_sinkgd_ncode16.py.

uv run python3 -m image_lagcodec.run_lagcodec --config image_lagcodec/configs/lagcodec_overfit1000_selfattn_sinkgd_ncode8.py
"""

run_name = "cifar_lagcodec_overfit1000_selfattn_sinkgd_ncode8"
