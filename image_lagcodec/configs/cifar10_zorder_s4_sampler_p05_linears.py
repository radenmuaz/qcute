...
row-major, RGB still contiguous per pixel). group_head_type="linears" everywhere (every
member of a group predicted independently in parallel, off one shared linear projection --
the cheapest option, baseline for the linears/ar/diffusion three-way comparison). Same
architecture/optimizer/cascade_rollout_prob/epochs_per_phase as p05 otherwise.

uv run python3 -m image_lagcodec.run_lagcodec_zorder --config image_lagcodec/configs/cifar10_zorder_s4_sampler_p05_linears.py
"""

run_name = "cifar10_zorder_s4_sampler_p05_linears"
