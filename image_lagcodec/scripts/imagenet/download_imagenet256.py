...
image to 256x256 using the exact method from openai/improved-diffusion's image_datasets.py
(canonical ImageNet256 preprocessing used across the density-estimation literature, incl. the
...
def main():
    ...
    p.add_argument("--out_dir", type=str, default="/dev/shm/imagenet256")
    p.add_argument("--shard_size", type=int, default=10_000, help="images per shard")
    ...
    def new_shard():
        ...
        path = os.path.join(
            args.out_dir, f"imagenet256_{args.split}_{shard_idx:05d}.npy"
        )
    ...
    path = new_shard()
    pbar = tqdm(ds, desc=f"imagenet256 {args.split}")
