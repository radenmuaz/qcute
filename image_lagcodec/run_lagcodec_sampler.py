...
CURRICULUM: n_phases = n_levels-1, phase p (1-indexed) grows the trainable set to levels[0..p-1]
(curriculum_mode="no_freeze" always, for this fork -- see below for why).

CASCADE ROLLOUT SAMPLER (chat 2026-09-12, forked from run_lagcodec_curriculum.py): unlike that
file (which only retrains the NEWEST level's decoder each phase, conditioned on its own real
encoder code), THIS file retrains EVERY active level's decoder every phase (levels[0..phase-1],
top-down), and each training STEP does a single global Bernoulli(cascade_rollout_prob) draw
choosing between two ctx sources for ALL of them at once:
  - "gt" (prob 1-p): every level i's decode() ctx is its own real encoder code (codes_soft[i]) --
    original behavior.
  - "cascade" (prob p): the topmost active level's ctx is still its own real code (nothing above
    it to substitute), but EVERY level below gets a CHEAP, non-autoregressive approximation of
    what would actually condition it at real cascade-generation time: level i's decode() is run
    teacher-forced (one parallel forward, not a sequential decode_generate loop) against the REAL
    target codes[i-1], and quantize_hard's argmax+STE trick is applied to those logits to make a
    "pseudo-generated" code -- THAT (not codes_soft[i-1]) becomes level (i-1)'s ctx. This chains
    top-down (level i-1's own ctx substitution then feeds level i-2, etc.), so in the "cascade"
    branch, simulation error compounds down the stack exactly like it does at real inference --
    still one JIT-compiled parallel forward pass overall, no Python-level sequential loop.
Every level must stay trainable (curriculum_mode="no_freeze" enforced) for this to make sense:
a level conditioned on the pseudo (noisier) ctx needs to be free to adapt its decoder to that
distribution, not frozen against it.

pmap-parallel across all local devices.

uv run python3 -m image_lagcodec.run_lagcodec_sampler --config image_lagcodec/configs/cifar10_curr_off_s4_sampler.py
"""
...
class Config:
    ...
    cascade_rollout_prob: float = 0.5   # chat 2026-09-12 -- per training STEP, probability of
    # using the chained cascade-simulated ctx (see module docstring) instead of real encoder ctx,
    # for every level below the topmost active one. 0.0 = always real ctx (degenerates to training
    # every level's decode() the plain way, still generalized to all levels though -- see
    # phase_forward). curriculum_mode is force-checked ("no_freeze" required) since a level
    # conditioned on cascade-simulated ctx must stay trainable to adapt to it.
    ...
    def __post_init__(self):
        ...
        assert self.curriculum_mode == "no_freeze", \
            "run_lagcodec_sampler requires curriculum_mode='no_freeze' -- a level conditioned on " \
...
def phase_forward(model: HierEncDec, flat_bytes: jnp.ndarray, phase: int, rng=None,
    ...
    """phase in 1..N-1. Runs encode() for levels 0..phase-1 (range(phase): top-down code chain,
    always REAL/ground-truth -- unaffected by use_cascade, only the DECODE ctx below is
    substituted). rng=None (eval/inference): quantize_hard everywhere. rng given (training):
    split one sub-key per level for encode()'s gumbel noise (quantize_mode="gumbel" only).

    decode() is now GENERALIZED to every level 0..phase-1 (not just the newest), trained top-down:
    the topmost active level (phase-1) always decodes conditioned on its OWN real code (nothing
    above it to substitute). use_cascade=None (or omitted): every level below also uses its own
    real code as ctx -- plain generalized teacher-forcing, no rollout. use_cascade=<traced bool
    scalar> (training, from a per-step Bernoulli draw): jnp.where-blends, per level, between real
    ctx and a CASCADE-SIMULATED ctx -- level i's decode logits (already computed for its own
    loss) get quantize_hard's argmax+STE applied, and THAT (not codes_soft[i-1]) becomes level
    (i-1)'s ctx candidate. This chains top-down: in the "cascade" branch every level's ctx is
    quietly built from the level above's simulated (not real) output, so simulation error
    compounds down the stack exactly like real cascade generation -- still one parallel/JIT'd
    forward pass, no sequential decode_generate loop. use_cascade must be a jax boolean (not a
    Python bool) since it's typically the traced output of jax.random.bernoulli under jit/pmap;
    jnp.where handles the branch instead of Python if/else."""
    levels = model.levels
    ...
    dec_losses, dec_accs = [], []
    ctx = codes_soft[phase - 1]   # topmost active level: always its own real code, nothing above
    ...
    byte_acc = dec_accs[-1]   # dec_accs built top-down (phase-1 .. 0) -- last entry is level0,
    # the byte-level accuracy, kept as the headline "dec_acc" metric (most interpretable, and the
    # one every phase actually has, unlike the newest-level accuracy convention used pre-fork).
...
def main():
    ...
    for phase in range(1, n_phases + 1):
        ...
        def train_step(diff_model, opt_state, rng, flat_bytes, static_model=static_model):
            rng, gumbel_rng, cascade_rng = jax.random.split(rng, 3)
            ...
            (loss, aux), grads = jax.value_and_grad(loss_fn, has_aux=True)(
                diff_model, static_model, flat_bytes, use_rng, use_cascade)
