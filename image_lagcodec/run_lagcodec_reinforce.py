...
CURRICULUM (chat 2026-09-12, forked from run_lagcodec_sampler.py, briefly ran no-curriculum
single-stage before this -- reinstated after w0.1/w0.2/w0.5/w1.0 no-curriculum ablations all
landed near-zero CASCADE_acc): phase p (1-indexed) grows the trainable set to levels[0..p-1],
epochs_per_phase epochs each, curriculum_mode="no_freeze" required (same as run_lagcodec_sampler).

REINFORCE OBJECTIVE (chat 2026-09-12): every level's decode() still trains via the STANDARD
teacher-forced pass (own real encoder ctx, codes_soft[i]) -- this loss is ALWAYS computed and
backpropped, non-negotiable. On top of it, an ADDITIONAL policy-gradient-style term, weighted by
Config.reinforce_weight (0 = disabled, 1.0 = same weight as the standard loss -- a plain scalar,
no Bernoulli gating): both the standard pass and the chained cascade-approx pass (reusing
run_lagcodec_sampler's cheap, non-autoregressive quantize_hard-argmax+STE ctx substitution -- NOT
real autoregressive sampling/decode_generate) are run every step; each pass's byte-level
(level0) accuracy becomes a reward (r2 for the standard/baseline pass, r1 for the cascade-approx
pass); advantage = stop_gradient(r1 - r2); the REINFORCE loss = (cascade-approx pass's own
cross-entropy loss) * advantage, added to the total loss at reinforce_weight. Intuition: when
the cascade-approx pass does BETTER than the real-ctx baseline (r1>r2, positive advantage), its
loss gets REWARDED (pushed down further); when it does worse, its loss gets PENALIZED (pushed
up) -- directly optimizing toward the accuracy metric via the policy-gradient identity, using
the paired real-vs-cascade rollout as a low-variance self-critical baseline, with zero actual
sampling anywhere (both passes are ordinary parallel forward passes).

pmap-parallel across all local devices.

uv run python3 -m image_lagcodec.run_lagcodec_reinforce --config image_lagcodec/configs/cifar10_curr_off_s4_reinforce.py
"""
...
class Config:
    ...
    reinforce_weight: float = 0.1   # chat 2026-09-12 -- plain scalar weight on the REINFORCE
    # term (see module docstring), NOT a probability -- 0.0 disables it entirely (pure standard
    # teacher-forced training), 1.0 gives it equal weight to the standard decode loss. Both the
    # standard and cascade-approx passes run EVERY step regardless of this value (it only scales
    # the extra term's contribution, doesn't gate whether it's computed).
    ...
    def __post_init__(self):
        ...
        assert self.curriculum_mode in ("freeze", "no_freeze")   # unused (no phase loop in this
        # fork -- kept only because phase_trainable_filter's signature still reads it, harmless)
        assert self.reinforce_weight >= 0.0
...
class EncDecLevel(eqx.Module):
    ...
    def decode_logits(self, target_seq: jnp.ndarray, ctx_code_soft: jnp.ndarray, lag: int) -> jnp.ndarray:
        ...
        supported. Split out from decode() (chat 2026-09-12) so the cascade rollout sampler can
        reuse these SAME logits both for this level's own loss AND (via quantize_hard's argmax+
        STE) as a cheap non-autoregressive approximation of what this level's decoder would
        generate -- fed downward as the level BELOW's substitute ctx (see module docstring)."""
        ...
        h_t = h[:, pred_pos, :]
        return self._dec_logits(h_t[:, :n_blocks * self.K, :])

    def decode(self, target_seq: jnp.ndarray, ctx_code_soft: jnp.ndarray, lag: int) -> tuple:
        logits = self.decode_logits(target_seq, ctx_code_soft, lag)
...
def phase_forward(model: HierEncDec, flat_bytes: jnp.ndarray, phase: int, rng=None) -> tuple:
    """phase grows 1..n_phases across the curriculum loop (see module docstring); levels[0..phase-1]
    are the active/trainable set each stage (phase_trainable_filter, no_freeze).
    Runs encode() for levels 0..phase-1 (top-down code chain, always REAL/ground-truth), then
    TWO decode passes over the SAME codes (see _decode_chain): "standard" (every level's own real
    ctx -- ALWAYS the loss that gets trained) and "cascade-approx" (chained quantize_hard-argmax+
    STE pseudo-ctx, cheap/non-autoregressive, ported from run_lagcodec_sampler.py). Each pass's
    own level0 byte-accuracy becomes a reward (r2 standard/baseline, r1 cascade-approx);
    advantage = stop_gradient(r1-r2); REINFORCE loss = cascade pass's own loss * advantage,
    added at Config.reinforce_weight. rng=None (eval/inference): quantize_hard everywhere in
    encode(). rng given (training): split one sub-key per level for encode()'s gumbel noise
    (quantize_mode="gumbel" only) -- unrelated to the two decode passes, which never sample."""
    levels = model.levels
    ...
    def decode_chain(use_cascade: bool) -> tuple:
        dec_losses, dec_accs = [], []
        ctx = codes_soft[phase - 1]   # topmost level: always its own real code, nothing above
        for i in range(phase - 1, -1, -1):
            dec_target = flat_bytes if i == 0 else codes[i - 1]
            logits = levels[i].decode_logits(dec_target, ctx, model.cfg.lag)
            loss_i, acc_i = levels[i]._dec_loss_acc(logits, dec_target)
            dec_losses.append(loss_i)
            dec_accs.append(acc_i)
            if i > 0:
                if use_cascade:
                    ctx, _ = quantize_hard(logits)
                else:
                    ctx = codes_soft[i - 1]
        return jnp.mean(jnp.stack(dec_losses)), dec_accs[-1]   # dec_accs[-1] = level0 (byte) acc

    dec_loss_std, r2 = decode_chain(use_cascade=False)
    dec_loss_cascade, r1 = decode_chain(use_cascade=True)
    advantage = jax.lax.stop_gradient(r1 - r2)
    reinforce_loss = dec_loss_cascade * advantage

    ntp_loss_total = jnp.mean(jnp.stack(enc_losses))
    main_loss = dec_loss_std + model.cfg.ntp_weight * ntp_loss_total
    # Conditioning scaler (chat 2026-09-12): cap |reinforce term| at |main_loss| -- like grad
    # clipping, only ever shrinks (never amplifies), stop_gradiented so it's a pure magnitude cap,
    # not a differentiable path back into the advantage. Needed because right at a phase
    # transition the freshly-added level's cascade ctx is garbage, r1 craters, and the raw
    # advantage/reinforce term spikes to 1-2x main_loss -- a single destructive update exactly
    # when the new level most needs a stable gradient (observed: w02 phase3 epoch10 term=-1.74
    # vs main_loss=0.88).
    raw_term = model.cfg.reinforce_weight * reinforce_loss
    scale = jax.lax.stop_gradient(jnp.minimum(1.0, jnp.abs(main_loss) / (jnp.abs(raw_term) + 1e-8)))
    reinforce_term = raw_term * scale

    loss = main_loss + reinforce_term
    bpb = dec_loss_std / jnp.log(2.0)
    return loss, (bpb, r2, ntp_loss_total / jnp.log(2.0), jnp.mean(jnp.stack(enc_accs)),
                  jnp.mean(jnp.stack(utils)), r1, advantage)
...
CONFIG_FIELDS = ("img_size", "d_model", "n_layers", "n_heads", "n_kv_heads", "strides",
                  ...
                  "gumbel_temperature", "gumbel_at_inference", "reinforce_weight")
...
def main():
    ...
    p.add_argument("--reinforce_weight", type=float, default=Config.reinforce_weight)
    ...
    # CURRICULUM (chat 2026-09-12, ported from run_lagcodec_sampler.py): phase p (1-indexed)
    # grows the trainable set to levels[0..p-1] (curriculum_mode="no_freeze" required, same as
    # the sampler file); REINFORCE dual-pass mechanics inside phase_forward are unchanged, just
    # now scoped to the phase's active levels each stage instead of all levels from step 1.
    step = 0
    for phase in range(1, n_phases + 1):
        ...
        def train_step(diff_model, opt_state, rng, flat_bytes, static_model=static_model):
            ...
            use_rng = gumbel_rng if cfg.quantize_mode == "gumbel" else None
            (loss, aux), grads = jax.value_and_grad(loss_fn, has_aux=True)(
                diff_model, static_model, flat_bytes, use_rng)
        ...
        active_desc = f"phase{phase}[levels0-{phase - 1},reinforce_weight={cfg.reinforce_weight}]"
        ...
        logger(f"=== {active_desc} done, no freeze (no_freeze mode) ===")
