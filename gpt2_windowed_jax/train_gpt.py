...
def main():
    ...
    p.add_argument("--window", type=int, default=-1, help="bounded local self-attention window, -1 = dense/unbounded")
    ...
    print(f"n_devices={n_devices}  batch_size(per-device)={B}  eval_batch_size(per-device)={B_eval}  seq_len={T}  "
          ...
          f"use_remat={args.use_remat}  window={args.window}")
    ...
    cfg = ModelConfig(pos_method=args.pos_method, block_size=T, vocab_size=50304,
                       ...
                       window=args.window, **MODEL_SHAPES[args.model])
