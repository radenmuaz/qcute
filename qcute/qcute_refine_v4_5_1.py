"""v4.5 fork: self track's decode conditions block (b+1) on block b's own code, packed inline into
one self-attention pass (no cross-attention for the self track -- eliminates v4.5's Stage-0 +
cross-attn-stage pipeline there). Block 0 skipped. Math: docs/qcute_refine_v4_4_1_v4_5_1_math.md.
Multi-track (coarser levels) unchanged from v4.5 (staged cross-attention, see docs/status.md).

    uv run python -m qcute.qcute_refine_v4_5_1 --config configs/qcute_refine_v4_5_1_overfit10k_k4single.py
"""
import argparse
import gzip
import json
...
class Config:
    ...
    decode_stage0_recompute: bool = False
...
class LevelLM(nn.Module):
    ...
    def selfcode_decode(self, seq_repr: torch.Tensor, code_kv: torch.Tensor, level: int, K: int,
        ...
        is_byte_level = level == 0
        if is_byte_level:
            x0 = self.embed(seq_repr)
            B, L = seq_repr.shape
        else:
            x0 = seq_repr @ self.embed.weight
            B, L, _ = seq_repr.shape
        H, hd = cfg.n_heads, D // cfg.n_heads
        device = x0.device
        n_blocks = L // K
        ...
        if compute_ntp:
            target = (seq_repr[:, K:].reshape(-1) if is_byte_level
                      else seq_repr[:, K:, :].argmax(-1).reshape(-1))
            logits = F.linear(query_h, self.embed.weight)
            ntp_loss = F.cross_entropy(logits, target)
            with torch.no_grad():
                ntp_acc = (logits.argmax(-1) == target).float().mean()
        else:
    ...
    def _run(self, byte_ids: torch.Tensor, compute_ntp: bool = True, max_decode_sources: int | None = None):
        ...
        for i in reversed(range(self.n_levels)):
            ...
            if len(full_track_specs) == 1:
                ...
                c_final, loss_final, acc_final, x_out = stage_lm0.selfcode_decode(
                    x_list[i], code_embeds0, i, track_K0, window0, compute_ntp=compute_ntp,
                    want_code=(max_decode_sources is None))
