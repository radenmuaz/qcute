...
`cfg.learned_query` (default False) is an OPTIONAL, separate mechanism: a single trainable query
vector, trained via one randomly-sampled position per batch (cheap, not a full per-position loss),
meant for a LATER post-training step toward block-parallel local decode (predicting several future
positions' bytes without needing their true sequential hidden states first) -- not required for,
or exercised by, the default training path.
...
class FuseShared(nn.Module):
    """"block fuse"'s MLP (+its pre-MLP LayerNorm), shared across EVERY fuse stage/period --
    only the cross-attention projections differ per stage (see FuseStage)."""
    def __init__(self, d_model: int, mlp_mult: int, n_layers: int):
        super().__init__()
...
class FuseStage(nn.Module):
    """"block fuse": cross-attention + MLP, used once per periodic-fusion stage. Own attention
    weights (ln1+attn); MLP (ln2+mlp) is `shared` (FuseShared) across every stage. Cheap: called
    ...
    def forward(self, x: torch.Tensor, code_kv: torch.Tensor, cos_q, sin_q, cos_k, sin_k,
                attn_mask: torch.Tensor, shared: FuseShared) -> torch.Tensor:
        for l in range(len(self.attn)):
            ...
            x = x + shared.mlp[l](shared.ln2[l](x))
        return x
...
class Config:
    ...
    learned_query: bool = False              # optional block-parallel-decode query vector, off by default
    learned_query_weight: float = 1.0
...
class QCuteZero(nn.Module):
    def __init__(self, cfg: Config):
        ...
        self.fuse_shared = nn.ModuleList([FuseShared(D, cfg.mlp_mult, fuse_layers) for _ in range(self.n_fuse)])
        self.fuse_stages = nn.ModuleList([FuseStage(D, cfg.n_heads, fuse_layers) for _ in range(self.n_fuse)])
        self.fuse_windows = resolve_fuse_window(cfg.fuse_window, self.n_fuse)

        if cfg.learned_query:
    ...
    def forward(self, byte_ids: torch.Tensor) -> tuple:
        ...
        code_kv_cache = []       # (h_code_s, code_pos_abs, window) per stage, reused by learned_query
        ...
        for s in range(self.n_fuse):
            ...
            x_cross = self.fuse_stages[s](x_cross, h_code, cos_q, sin_q, cos_k, sin_k, fuse_mask, self.fuse_shared[s])

            cond_h = self.ln_f(x_cross)
            cond_logits = F.linear(cond_h[:, :-1, :], self.embed.weight)
        ...
        # --- optional: learned generic query vector (block-parallel-decode groundwork) ---
        learned_query_loss = None
        if cfg.learned_query and self.n_fuse > 0 and L >= 2:
            q_idx = int(torch.randint(1, L, (1,)).item())
            qv = self.query_vec.view(1, 1, D).expand(B, 1, D)
            xq = qv
            cos_q1, sin_q1 = rope_cos_sin_for_positions(torch.tensor([q_idx], device=device), hd, cfg.rope_base, device)
            for s, (h_code, code_pos_abs, window_s) in enumerate(code_kv_cache):
                mask_q = causal_mask(torch.tensor([q_idx], device=device), code_pos_abs, window_s)
                cos_k, sin_k = rope_cos_sin_for_positions(code_pos_abs, hd, cfg.rope_base, device)
                xq = self.fuse_stages[s](xq, h_code, cos_q1, sin_q1, cos_k, sin_k, mask_q, self.fuse_shared[s])
            lq_logits = F.linear(self.ln_f(xq)[:, 0, :], self.embed.weight)
            learned_query_loss = F.cross_entropy(lq_logits, byte_ids[:, q_idx])
        ...
        if learned_query_loss is not None:
            total_loss = total_loss + cfg.learned_query_weight * learned_query_loss
        ...
        if learned_query_loss is not None:
            metrics["learned_query_loss"] = learned_query_loss
    ...
    def _forward_next_byte_logits(self, byte_ids: torch.Tensor) -> torch.Tensor:
        ...
        (position L, i.e. the last position's post-fusion prediction). Reuses forward()'s own
        cascade unchanged -- no separate generation-time code path to drift out of sync with
        training, unlike qcute_v1's generate_no_cache/_stack_generate_blockwise split (see
        ...
        for s in range(self.n_fuse):
            ...
            fuse_mask = causal_mask(byte_pos, code_pos_abs, window_s)
            cos_k, sin_k = rope_cos_sin_for_positions(code_pos_abs, hd, cfg.rope_base, device)
            x_cross = self.fuse_stages[s](x_cross, h_code, cos_b, sin_b, cos_k, sin_k, fuse_mask, self.fuse_shared[s])
            cur_h = h_code

        cond_h = self.ln_f(x_cross)
        return F.linear(cond_h[:, -1, :], self.embed.weight)
...
def build_argparser(description: str) -> tuple:
    ...
    p.add_argument("--learned_query", action="store_true", default=False)
    p.add_argument("--learned_query_weight", type=float, default=1.0)
...
def config_from_args(args) -> Config:
    return Config(
        ...
        learned_query=args.learned_query, learned_query_weight=args.learned_query_weight,
    )
