...
class Attn(nn.Module):
    """Shared QKV/out projections for self- (forward) and cross-attention (forward_cross). GQA
    (n_kv_heads < n_heads repeats each KV head across n_heads//n_kv_heads query heads) + optional
    Qwen3-style QK-norm (per-head RMSNorm on Q/K before RoPE) + optional decoupled head_dim
    (Qwen3-style: some of its smaller variants set head_dim independently of d_model//n_heads --
    None here is a no-op, deriving head_dim the old way)."""
    ...
    def forward_incremental(self, x_new: torch.Tensor, cos_new: torch.Tensor, sin_new: torch.Tensor,
        ...
        """x_new: only the NEW position(s). cache: None or (k_prev, v_prev), stored PRE-repeat
        (n_kv_heads heads). Mask uses LOCAL (call-relative) positions -- cos/sin (true absolute
        positions) is what encodes real distance."""
        B, Tn, D = x_new.shape
        H, Hkv, hd = self.n_heads, self.n_kv_heads, self.head_dim
