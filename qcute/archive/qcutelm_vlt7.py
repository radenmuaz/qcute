"""qcute.qcutelm_vlt7 — hybrid: symmetric narrow tokenizer at the edges,
heavy separate-weight codelm in the middle, operating on the short code
sequence.

Lineage within this file (kept, not a new fork, per "v7 direct"):
...
KNOWN GAP, now fixed: v1/v2/early-v3 had no windowing/chunking on the
tokenizer tier's O(T^2) attention. `attn_window` (Config, CLI, per-config)
now ports qcutelm_vlt6's verified _forward_chunked_no_sink mechanism onto
the tokenizer tier's Blocks — O(T*window) instead of O(T^2), same math,
falls back to dense automatically when the sequence length isn't a clean
multiple of the window (needed for generate()'s growing sequences).
codelm's own attention is left dense (its sequence is already short,
windowing it isn't the point).
...
convention) — Logger/Checkpointer/schedule helpers/quantizers duplicated.
    ...
    uv run python -m qcute.qcutelm_vlt7 --config configs/qcutelm_vlt7_<name>.py
"""
import argparse
import gzip
import json
...
class Config:
    ...
    shared_tokenizer_phases: bool = True  # True (original): Pass 1 ("no-code") and Pass 2 ("forecast") reuse
                                           # the SAME byte_emb/blocks/ln_f/head weights. False: Pass 2 gets its
                                           # own independent copies (same architecture, untied params) —
                                           # mirrors qcutelm_vlt6's shared_encoder_decoder=False.
...
class InterleavedSymmetricLM(nn.Module):
    ...
    def forward(self, ctx: torch.Tensor) -> tuple[torch.Tensor, dict]:
        ...
        z_hat = self.quantize(self.code_pre(slot_hidden))   # [B, n_blocks, dq]
...
def train(model: InterleavedSymmetricLM, train_data: torch.Tensor, val_data: torch.Tensor, args, log, run_name: str, device: str) -> None:
    ...
    model.train()
    pbar = tqdm(range(1, args.steps + 1), desc="train_vlt7", dynamic_ncols=True)
    for step in pbar:
...
def main():
    ...
    p = argparse.ArgumentParser(description="Symmetric narrow tokenizer + separate wide codelm on the short code sequence (fork of qcute.qcutelm_vlt6)", parents=[pre])
    ...
    p.add_argument("--shared_tokenizer_phases", type=lambda x: x.lower() != "false", default=True)
    ...
    run_name = args.run_name or (pre_args.config.stem if pre_args.config else f"qcutelm_vlt7_{int(time.time())}")
