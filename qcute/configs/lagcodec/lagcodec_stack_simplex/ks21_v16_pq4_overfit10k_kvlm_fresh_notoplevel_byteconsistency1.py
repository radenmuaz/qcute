...
(new Config.byte_consistency_p, qcute_lagcodec.py, 2026-08-23). Real cost: this doubles the whole
forward pass (every level's encoder + decode), not just decode_level, whenever it fires.

uv run python -m qcute.qcute_lagcodec.qcute_lagcodec --decoder_type stack --config configs/lagcodec/lagcodec_stack_simplex/ks21_v16_pq4_overfit10k_kvlm_fresh_notoplevel_byteconsistency1.py
