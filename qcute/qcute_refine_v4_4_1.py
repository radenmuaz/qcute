"""v4.4 fork: self track's decode conditions block (b+1) on block b's own code (LM continuation,
not autoencoder). Block 0 skipped (nothing precedes it). Math: docs/qcute_refine_v4_4_1_v4_5_1_math.md.
Multi-track (coarser levels) unchanged from v4.4.

    uv run python -m qcute.qcute_refine_v4_4_1 --config configs/qcute_refine_v4_4_1_overfit10k_k4single.py
"""
import argparse
import gzip
import json
...
class LevelLM(nn.Module):
    ...
    def _packed_decode_forward_selfcode(self, x0: torch.Tensor, code_kv: torch.Tensor, K: int,
                                         window: int | None) -> tuple[torch.Tensor, int]:
    ...
    def forward(self, seq_repr: torch.Tensor, level: int, window: int | None, compute_ntp: bool = True,
        ...
        if decode_tracks is not None:
            ...
            if len(decode_tracks) == 1:
                ...
                n_blocks_sc = L // K
                he, n_units = self._packed_decode_forward_selfcode(x0, code_kv0, K, w0)
