"""qcute_v5_concat: now the default v5 concat module (promoted from qcute_v5_concat_skip), forked
from qcute_v5_concat_fixblock.py, adding one more pruning on top of
...
**qcute_v5_concat_modes: forked from qcute_v5_concat.py to add multi-mode decode loss** --
...
stages (decode_stage_extra_losses). qcute_v5_concat.py's decode has no such byproduct -- it's ONE
flat self-attention pass over a merged buffer holding every track's codes at once, so there's no
natural intermediate readout to tap. Config.multi_mode_impl selects how (or whether) to get those
same per-depth losses here:
  - "off" (default): unchanged qcute_v5_concat.py behavior, one loss per level (the full/deepest
    mode only), zero overhead.
  ...
  - "single_pass": one shared self-attention pass gets every mode's loss. The T=len(tracks) mode
    (self+every coarser track) uses the real byte buffer positions exactly as qcute_v5_concat.py
    always has (via extract_pos). Each SHALLOWER mode m<T gets its own block of L QUERY-ONLY buffer
    entries appended after the real backbone -- one entry per byte position b, embedded from the
    same x0 byte embeddings, sharing every layer's qkv projection weights with the backbone, but
    masked so it (a) may only see backbone entries with true_pos<=b AND category<=m (byte=category
    0 always counts; track j's code is category j+1), reusing the same category-based masking
    primitive qcute_v5_concat.py's own `_skip` extension introduced, and (b) is NEVER visible as a
    key to anything -- not the backbone, not other modes' query-copies, not even other bytes' copies
    within its own mode -- so it can never contaminate the real computation, only read from it. This
    is why duplicate query rows are necessary at all: unlike a code tied at the SAME true_pos as a
    query (already naturally isolated from later/higher-category entries by ordinary causal
    ordering, no trick needed), a byte LATER in the sequence, in the shared backbone pass, causally
    sees every earlier code regardless of category -- there's no way to selectively blind an
    existing buffer entry to only SOME of its causal past without giving it a separate, appropriately
    masked copy of itself. Scoped to the dense (non-chunked) attention path only -- the chunked/
    banded path's own windowed-chunk gather machinery isn't extended to carry query-copy segments
    (a real limitation, not a correctness shortcut: chunked configs with multi_mode_impl="single_pass"
    are unsupported, not silently wrong -- caught by construction). Verified against "multipass"
    (scripts/test_v5_concat_modes.py) across Ks=(1,), (4,1), (2,2,1): all-mode logits/losses match
    exactly under quant_type="softmax" code_sample_mode="ste" (the only combination that's
...
class LevelLM(nn.Module):
    ...
    def forward(self, seq_repr: torch.Tensor, level: int, window: int | None, compute_ntp: bool = True,
        ...
        return c_i, ntp_loss, ntp_acc, h, query_last, None, mode_losses, mode_accs
...
class RefineLM(nn.Module):
    ...
    def _run(self, byte_ids: torch.Tensor, compute_ntp: bool = True, max_decode_sources: int | None = None,
        ...
        for i in range(self.n_levels):
            ...
            c_i, loss_i, acc_i, h_i, _, _, _, _ = self.encode_lms[i](seq_repr, level=i, window=self.windows[i], compute_ntp=want_ntp)
    ...
    def forward(self, byte_ids: torch.Tensor) -> tuple[torch.Tensor, dict]:
        ...
        (encode_losses, encode_accs, decode_losses, decode_accs, h_list, c_list,
         ...
         decode_stage_extra_losses) = self._run(byte_ids)
...
def generate_no_cache(model: "RefineLM", prompt_bytes: torch.Tensor, n_new_bytes: int, device: str,
    ...
    for _ in range(n_new_bytes):
        ...
        _, _, _, _, h_list, _, next_query, _decode_derived_c, _query_seq, _, _ = model._run(
...
def generate_encode_only(model: "RefineLM", prompt_bytes: torch.Tensor, n_new_bytes: int, device: str) -> torch.Tensor:
    ...
    for _ in range(n_new_bytes):
        _, _, _, h, _, _, _, _ = enc0(all_bytes, level=0, window=model.windows[0], compute_ntp=False)
...
def generate_level1_codes(model: "RefineLM", prompt_bytes: torch.Tensor, n_new_codes: int, device: str) -> torch.Tensor:
    ...
    codes, _, _, _, _, _, _, _ = enc0(prompt_bytes, level=0, window=model.windows[0], compute_ntp=False)
    n_prompt_codes = codes.shape[1]
    for _ in range(n_new_codes):
        _, _, _, h1, _, _, _, _ = enc1(codes, level=1, window=model.windows[1], compute_ntp=False)
...
def level1_ground_truth_codes(model: "RefineLM", full_bytes: torch.Tensor, prompt_len: int, device: str) -> torch.Tensor:
    ...
    enc0 = model.encode_lms[0]
    K0 = model.cfg.Ks[0]
    c0, _, _, _, _, _, _, _ = enc0(full_bytes, level=0, window=model.windows[0], compute_ntp=False)
...
def _decode_source_codes(model: "RefineLM", full_bytes: torch.Tensor, device: str, level: int = -1) -> torch.Tensor:
    ...
    for i in range(model.n_levels):
        c_i, _, _, _, _, _, _, _ = model.encode_lms[i](seq_repr, level=i, window=model.windows[i], compute_ntp=False)
        c_list.append(c_i)
        seq_repr = c_i
...
def check_gen_consistency(model: "RefineLM", full_bytes: torch.Tensor, device: str,
    ...
    _, _, _, _, h_list_tf, _, _, _, query_seq_tf, _, _ = model._run(
    ...
    for t in range(prompt_len, L_total - 1):
        ...
        if ref_idx < 0 or ref_idx >= logits_tf_all.shape[0]:
            continue
        padded = full_bytes[:, :t]
        _, _, _, _, h_list_gen, _, next_query_gen, _, _, _, _ = model._run(
