"""qcute.qcute_lagcodec.qcute_lagcodec_wordlm — plain unconditional next-word LM baseline (the bytelm.py job),
ported to run through qcute_lagcodec's own shared training infra instead of a standalone module, so
its logs/checkpoints/run.jsonl are directly comparable to every other qcute_lagcodec config with no
...
uv run python -m qcute.qcute_lagcodec.qcute_lagcodec_wordlm --config configs/v1_word/xs.py
"""
from types import SimpleNamespace

import torch.nn as nn

from qcute.qcute_lagcodec.qcute_lagcodec_common import Config, resolve_per_level, run_main
