...
class Config:
    ...
    # see qcute_lagcodec.py). "pervasive": every level's own decode_level runs and contributes to
...
class CausalSelfAttention(nn.Module):
    ...
    optional Qwen3-style QK-norm (per-head RMSNorm on Q/K before RoPE). qcute_lagcodec_decoder.py's own
    ...
    `hd = D // cfg.n_heads`-style local var in qcute_lagcodec_decoder.py's own ~13 manual call sites
    (and qcute_lagcodec_common.py's LM/rope call sites) updated to read a resolved head_dim instead --
    ...
    def __init__(self, d_model: int, n_heads: int, n_kv_heads: int | None = None, qk_norm: bool = True):
        ...
        # kept as one combined Linear (not split wq/wk/wv) so qcute_lagcodec_decoder.py's raw
...
class LM(nn.Module):
    def __init__(self, cfg: Config, d_model: int, n_layers: int, vocab: int):
        ...
        # trainable per-block SEED TOKEN, prepended before every K-block in qcute_lagcodec's non-top-level
        # decode (see docs/qcute_lagcodec_plan.md) -- not "BOS" (a single sequence-start marker) or a
...
def self_code_active(level: int, n_levels: int, use_self_code: bool) -> bool:
    ...
    NTP loss (see qcute_lagcodec.QCuteLM.forward's decode_losses[0] fallback)."""
...
def warn_degenerate_self_code(level: int) -> None:
    ...
    print(f"WARNING: level{level} use_self_code=False with n_levels==1 -- decode degenerates to "
          f"an unconditioned LM (encode-only fallback, equivalent to qcute_lagcodec_wordlm). Confirm intended.")
...
def run_main(QCuteLM) -> None:
    args, pre_args = build_argparser("qcute_lagcodec: shared Encoder, pluggable concat/stack Decoder")
    ...
    run_name = args.run_name or (pre_args.config.stem if pre_args.config else f"qcute_lagcodec_{int(time.time())}")
