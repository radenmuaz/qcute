...
from qcute.qcute_lagcodec.qcute_lagcodec_common import (
...
def bos_interleaved_self_attn(bb: LM, x0: torch.Tensor, K: int, window: int | None,
    ...
    """qcute_lagcodec non-top-level decode self-attention (see docs/qcute_lagcodec_plan.md): prepends
...
def own_block_cross_attn_decode(bb: LM, h_self_full: torch.Tensor, code_kv: torch.Tensor,
    ...
    per docs/qcute_lagcodec_plan.md's "causality is enforced by where the conditioning code comes from"
...
def split_track0_window(track0_window) -> tuple:
    ...
    qcute_lagcodec.py's _norm_track0). Every call site that used to write `track0_window, track0_window`
...
def encode_like_self_attn_decode(bb: LM, x0: torch.Tensor, code_kv: torch.Tensor, K: int,
    ...
    docs/qcute_lagcodec_plan.md and chat that day): plain causal self-attention over the real byte
...
def bos_query_only_parallel_sync_decode(bb: LM, x0: torch.Tensor, K: int) -> torch.Tensor:
    ...
    this is block-*parallel*, synchronized on offset within block. See docs/qcute_lagcodec_plan.md's
...
class Decoder(nn.Module):
    ...
    def check_roundtrip_consistency(self, model, full_bytes: torch.Tensor, device: str, log=print,
                                     label: str = "") -> int:
        """Baseline metric 1 from docs/qcute_lagcodec_plan.md's generation-feasibility section: decode
    ...
    def check_decode_modes(self, model, full_bytes: torch.Tensor, device: str, log=print,
                            label: str = "") -> dict:
        """Dual-mode generation-quality proxy from docs/qcute_lagcodec_plan.md's generation-feasibility
    ...
    def qualitative_generate(self, model, prompt_bytes: torch.Tensor, gen_len: int,
        ...
        # see qcute_lagcodec_common.py's Config.active_srcs_mode docstring (renamed from
...
class StackDecoderV1(Decoder):
    """qcute_lagcodec: only the top level is a genuine NTP/AR decoder (self-code recurrence,
    ...
    byte). See docs/qcute_lagcodec_plan.md's worked example (c1 reconstructs 'ab', c2 reconstructs 'cd',
...
class StackDecoder(Decoder):
    """qcute_lagcodec (2026-08-20, "every upper code conditioning"): "like v5's [old] StackDecoder except
    ...
    changes needed in qcute_lagcodec.py). `decode_windows[i][t]` for t>=1 is track t's (level i+t+1's
