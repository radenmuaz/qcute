"""qcute.bpelm_fineweb — GPT-2-BPE-vocab fork of qcute.bytelm_fineweb (itself forked from
qcute.bytelm_tpu) for FineWeb-Edu (sample-10BT) training.

Standalone on purpose (not an import of qcute.bytelm_fineweb) so it can be read/debugged/deployed
to a TPU VM without dragging in the rest of the repo's device-selection assumptions, matching the
sibling module's own convention.

**"GPT-2-style" here means vocab/tokenizer only** — GPT-2's own 50257-token BPE vocabulary
(tiktoken's "gpt2" encoding) with a tied output head (head 0 already ties to `tok_emb.weight`,
exactly GPT-2's own design, so no separate head class was needed — just the vocab-size switch).
This is NOT a literal GPT-2 architecture clone and does NOT load GPT-2's pretrained weights: the
trunk stays this repo's own RoPE/RMSNorm/SwiGLU/MTP-heads stack (see qcute.bytelm_tpu's docstring),
not GPT-2's learned-position-embedding/LayerNorm/GELU/no-MTP design.

**Differences from qcute.bytelm_fineweb** (see that module's own docstring for everything not
listed here — model architecture, flash-attention, multichip, KV-cache generation, training loop,
metrics convention (loss primary / bpb secondary) etc. are unchanged):

  - **Vocab**: `LMConfig.vocab` defaults to 50257 (GPT-2's tiktoken vocab size), not 256.
  - **Data**: `--train_data`/`--val_data` point at flat `uint16` binaries produced by
    `scripts/prep_fineweb_edu_gpt2.py` (GPT-2-BPE token ids, `<|endoftext|>` inserted after every
    document as a real separator — unlike the byte-level sibling, there is no "no separator"
    option here, since GPT-2's own tokenizer already reserves an id for exactly this), mmap'd via
    `load_token_bin` (`np.memmap`, dtype uint16, mode="r") instead of `load_byte_bin`'s uint8.
  - **Generation**: `qualitative_generate`/CLI qual-gen paths decode/encode through tiktoken's
    "gpt2" encoding instead of treating ids as raw bytes — prompts and generated output are
    printed as decoded text, not `bytes(...)` reprs. `--qual_user_text` is BPE-encoded via
    `tiktoken.encode_ordinary`, not `str.encode("utf-8")`.
  - "bpb" (bits-per-byte) throughout this module is really **bits-per-token** (loss / ln(2))
    since the underlying unit is now a BPE token, not a raw byte — kept the same field/variable
    names as the byte-level sibling for log-format/plot_run.py compatibility, but the two are not
    directly comparable across the two modules without knowing bytes-per-token.
...
FineWeb-Edu sample-10BT is ~10^10 bytes, considerably fewer GPT-2 BPE tokens (~4 bytes/token
typical for English web text) — watch actual it/s on the very first run and retune `--steps` from
there, per this repo's standing "long runs have shown unpredictable throughput" caution (CLAUDE.md).

    uv run python -m qcute.bpelm_fineweb --config configs/bpelm_fineweb/small_d512x8_mlp4_1epoch_ctx1024_flash.py
"""
...
def load_token_bin(path: Path) -> np.memmap:
    # mode="r" -- mmap'd, not loaded into RAM. scripts/prep_fineweb_edu_gpt2.py writes these as a
    # flat uint16 GPT-2-BPE token id stream; random windows are gathered straight off disk (OS
    # page cache handles hot pages), since the corpus doesn't fit as one in-RAM tensor.
...
@dataclass
class LMConfig:
    vocab: int = 50257  # GPT-2 tiktoken vocab size (includes <|endoftext|>, id 50256)
    d_model: int = 1024
    n_layers: int = 8
    n_heads: int = 16
    context: int = 2048
...
PRESETS: dict[str, LMConfig] = {
    # ~(4 + 3*mlp_mult) * d_model^2 * n_layers non-embedding params (vocab=50257 is NOT negligible
    # here, unlike the byte-level sibling -- adds ~2*50257*d_model params, tied head0 shares one
    # of those two embedding tables) --
...
@torch.no_grad()
def score_continuation_bpb(model: ByteLM, full_tokens: list[int], prompt_len: int, device: torch.device) -> float:
    model.eval()
    seq = torch.tensor([full_tokens], dtype=torch.long, device=device)
...
def qualitative_generate(
    model: ByteLM, prompt_tokens: list[int], gen_len: int, ground_truth: list[int] | None, device: torch.device,
    ...
    gen_tokens = out[0, prompt.size(1):].tolist()

    log(f"qual_prompt:       {_GPT2_ENC.decode(prompt_tokens)!r}")
    log(f"qual_generated:    {_GPT2_ENC.decode(gen_tokens)!r}")
    if ground_truth is not None:
        log(f"qual_ground_truth: {_GPT2_ENC.decode(ground_truth)!r}")
        bpb = score_continuation_bpb(model, prompt_tokens + ground_truth, len(prompt_tokens), device)
...
def main():
    ...
    p = argparse.ArgumentParser(
        description="GPT-2-BPE-vocab causal transformer + MTP-head LM on FineWeb-Edu (loss primary, bpb secondary), CPU/TPU only", parents=[pre]
    )
...
def _run(index: int, args: argparse.Namespace, pre_args: argparse.Namespace) -> None:
    ...
    torch.manual_seed(args.seed)  # covers weight init, layer_drop/dropout masks, batch_iter sampling
    ...
    train_data = load_token_bin(args.train_data)
    val_data = load_token_bin(args.val_data)
    ...
    if not args.eval_only:
        ...
        log(f"~{steps_per_epoch:.1f} steps/epoch  ~{epochs:.1f} epochs over train_tokens "
    ...
    if args.benchmark_generate_bytes > 0:
        prompt = next(val_iter)[:1, :1]  # one real token as prompt, batch size 1
        benchmark_generation(model, prompt, args.benchmark_generate_bytes, log=log)

    if args.qual_gen_bytes > 0:
        if args.qual_source == "user":
            prompt_tokens = _GPT2_ENC.encode_ordinary(args.qual_user_text)
            ground_truth = None
        else:
            ...
            prompt_tokens = window[: args.qual_prompt_bytes]
            ground_truth = window[args.qual_prompt_bytes :]
        qualitative_generate(model, prompt_tokens, args.qual_gen_bytes, ground_truth, device, log=log)
