"""qcute.qcute_refine_v5 — CLONE of qcute_refine_v4_2.py (byte-for-byte
identical below this section as of the clone), carrying a DESIGN SKETCH
ONLY — session ask: "clone v4 to v5, sketch design to use h net pairwise
compare idea to decide where to pick h to generate bsq code, level 1 and
above takes input with variable stride not fixed k, k becomes a time
function of level 0." NOT YET IMPLEMENTED — every class/function below
this docstring is still v4.2's own code, unmodified; this section records
the intended change and exactly where it lands, matching
[docs/hierarchical_fusion_designs.md](../docs/hierarchical_fusion_designs.md)'s
own "design space, not a result" convention, extended here with a
concrete grounding in this file's actual classes/line-level mechanics
(that file's own H-Net discussion stayed abstract/architecture-agnostic;
this section is the point where it gets nailed down to `LevelLM.forward`
specifically).

**Where the fixed-K mechanism actually lives** (`LevelLM.forward`, this
file, currently unchanged from v4.2): `n_blocks = L // K` then
`h_blocks = h[:, :n_blocks*K, :].view(B, n_blocks, K, D)` then
`pre_q = self.code_pre(h_blocks[:, :, K-1, :])` — i.e. level `i`'s own
code handoff to level `i+1` is generated by reading h at ABSOLUTE FIXED
STRIDE `K = cfg.Ks[level]`, always the last position of every K-sized
block, unconditional on content. This is exactly the "flat `Ks` grid,
fixed-stride" limitation contrasted against H-Net's dynamic chunker
throughout `hierarchical_fusion_designs.md`.

**The sketch**: replace that fixed slice-and-view with a content-adaptive
boundary decision, reusing every mechanism this session already worked
out (in order of what plugs in where):

1. **Boundary scoring** — a small `self.boundary_head` scoring
   `sim(h_t, h_{t+1})` (H-Net's own original pairwise form, the `K=2`
   degenerate case of the windowed generalization) or, per the windowed
   extension discussed this session, a multi-lag/multi-offset score over
   a bounded window (reusing `BitPredictHeadConvDilated._dilated_stack`'s
   own `unfold`+`einsum` pattern for the windowed read). STE hard-select
   (same `pq_table` hard+soft-gradient pattern this file already uses
   for `CodeEmbed`) turns the score into a per-position `is_boundary`
   flag, with `p_conf` (winning-candidate confidence) carried forward for
   the decode-side gate — see point 4.
2. **Compress (parallel, no Python loop)** — `chunk_id =
   cumsum(is_boundary, dim=1)`, then either the scatter-based or the
   argsort-based (2-expert-MoE-style: `perm = argsort(is_boundary,
   stable=True)`, slice the contiguous "kept" block) gather into `c_i`,
   whose shape is now `[B, n_chunks_padded, dq]` with a validity mask —
   `n_chunks` is genuinely PER-BATCH-ROW variable now, not `L // K`. This
   is precisely "K becomes a time function of level 0": the raw-byte
   distance between consecutive selected positions is no longer a
   constant `Ks[level]`, it's whatever the boundary detector decided for
   that specific span — level `i+1`'s own effective stride is a function
   of level `i`'s content, not a config constant.
3. **What this breaks and needs generalizing, stated explicitly rather
   than glossed over** (this is why it's a sketch, not a diff):
   - `seq_lens` (`RefineLM.__init__`, currently a precomputed
     `[context_len, context_len // Ks[0], ...]` list asserted exact) can
     only be an upper bound per level now — every level above 0 needs
     padding + a validity mask carried through everything downstream
     (loss reduction, `eval_model`'s bpb aggregation), the same
     `PAD_BYTE`/`pad_mask` convention `hierarchical_fusion_designs.md`'s
     own tiling section already establishes, reused here for a different
     reason (data-dependent chunk COUNT within one document, not a
     document-length remainder).
   - `jagged_causal_mask_and_positions` and RoPE position tagging
     (`rope_cos_sin_for_positions`) are parameterized by a constant `K`
     throughout this file — every call site needs the ACTUAL resolved
     boundary position (from `chunk_id`/the gather index) instead of the
     assumed `(b+1)*K - 1`, for every level above 0.
   - `generate_kv_cache` currently knows exactly when a new code is
     "due" (every `K` raw bytes, a compile-time-known cadence) to decide
     when to advance level 1. With `K(t)`, generation must run the SAME
     boundary detector live, causally (bounded lookahead per the
     buffering tradeoff already discussed for the windowed extension),
     to decide code-emission timing on the fly — a genuine train/
     generation mechanism addition, not just a training-time change.
   - Training-time parallelism is NOT a new problem — the compress step
     above is exactly the mechanism already worked out end-to-end this
     session (cumsum + scatter/argsort-gather), directly reusable
     verbatim; nothing about variable stride reintroduces a sequential
     training loop.
4. **Decode/broadcast, if this level also FUSES (v4.2's own concat
   mechanism)** — `fuse_kv` currently reads `h_list[i+1]` directly,
   implicitly assuming a fixed alignment between level `i`'s raw
   positions and level `i+1`'s block-scaled ones. With variable stride,
   broadcasting level `i+1`'s state back down needs the H-Net-style
   dechunk gather (`upsampled = upper_out[chunk_id]`) before concat, and
   the STE confidence `p_conf` from point 1 is the natural gate weight
   for blending broadcasted-vs-local information at each raw position —
   same role H-Net's own boundary probability plays at its own decode
   step, carried over here rather than invented fresh.
5. **Recursion, not just level 0** — applying this at every level (not
   only the level-0-to-1 handoff) is exactly what turns this file's own
   fixed `Ks` tower into H-Net's recursively-stacked content-adaptive
   hierarchy, the correction this session's own H-Net discussion already
   made to the original single-level framing. Nothing about the sketch
   above is level-0-specific; `LevelLM.forward` already treats every
   level uniformly (v4.2's whole point), so the boundary mechanism would
   too.
6. **Passing chunk-length information UP to the level above — fixed `K`
   gave this for free, variable stride doesn't.** Under a constant `K`,
   level `i+1` implicitly knows every one of its own tokens represents
   exactly `K` raw positions — nothing needs to be told. Under `K(t)`,
   two of level `i+1`'s consumers need this and get it differently:
   - **Attention-based consumers** (its own self-attention, and the
     fusion cross-attention reading it) get it FOR FREE, but only
     indirectly/relationally: `sort_idx` (point 2's static-shape sketch)
     already tags every selected code with its REAL raw-byte position for
     RoPE, so the raw-byte gap between chunk `j` and chunk `j-1` is
     exactly what RoPE's own relative rotation between those two
     positions already encodes — but only if the model LEARNS to attend
     to its immediate predecessor to extract it, an indirect/learned
     signal, not a free one (the same "give it for free vs. make it
     learn it" tradeoff already flagged for raster-order 2D locality in
     `hierarchical_fusion_designs.md`).
   - **Non-attention consumers** — critically, `code_pre`/`bsq_quantize`
     ITSELF runs on `h_selected` before any of level `i+1`'s own
     `self.blocks` ever execute, so it has no access to that learned
     relational signal at all. Fix: an EXPLICIT, exact (not
     inferred/learned) length feature, computed directly from `sort_idx`
     since it's already in hand — `delta_pos = sort_idx - shift_right(sort_idx,
     fill=-1)` (each chunk's raw-byte gap since the PREVIOUS selected
     chunk, causal and exact by construction, zero extra machinery) —
     bucketed/log-scaled and summed into the level-above's own input
     embedding: `x = self.embed(x_in) + self.length_embed(bucket(delta_pos))`,
     the same "add an auxiliary embedding to the token embedding" pattern
     position embeddings already use elsewhere. Cheap, exact, no
     dependence on the model learning anything — the recommended default.
   - **A softer, more H-Net-like alternative, flagged explicitly as a
     best-effort reconstruction from memory, NOT verified against the
     paper** — instead of `h_selected` hard-picking a single position's
     `h` per chunk (point 2's `torch.gather`), accumulate a causal EMA
     across the span since the last boundary, gated by the boundary
     confidence (`s_t = decay(p_t)*s_{t-1} + (1-decay(p_t))*h_t`), and use
     the EMA's own state AT the boundary as the chunk summary instead of
     a raw single-position read. A longer span means the running average
     has "settled"/integrated more raw positions before being sampled —
     length shows up implicitly in the SUMMARY's own statistical
     character rather than as a separate channel, closer to what this
     session's H-Net discussion attributed to its smoothing module. Softer
     signal, real implementation risk (an EMA's "settledness" isn't a
     clean, monotonic proxy for span length the way `delta_pos` is exact),
     and uncertain fidelity to the actual paper mechanism — documented as
     a follow-up worth trying if the explicit feature above proves
     insufficient, not the starting point (same "prefer simple/exact
     first" ordering the cap-vs-ratio-loss split above already used).
7. **Cross-attention fusion masking now needs the REAL boundary
   positions, not a `K`-formula — training and generation diverge here in
   a genuinely useful way.**
   - **Training** (`LevelLM._prep_concat`/`jagged_causal_mask_and_positions`,
     currently a closed-form function of constant `K`): replace with a
     direct batched comparison against `sort_idx` — level-`i` raw
     position `t` may attend to level-`(i+1)` chunk `j` only if that
     chunk's own resolved boundary position is `<= t` (it's causally
     "in the past") AND the slot is real: `disallow = (sort_idx.unsqueeze(1)
     > t_positions.unsqueeze(2)) | (~valid).unsqueeze(1)` — a single
     batched broadcast comparison, same parallel shape as the existing
     mask builder, just data-driven instead of formula-driven. Still one
     `torch.compile`-friendly op over the STATIC `[B, L, C]` shape (point
     2's `C`), nothing dynamic reintroduced.
   - **Generation, and why it's actually SIMPLER, not harder** — this is
     the plain-pairwise (`K=2` degenerate) case's genuine advantage over
     the windowed/lookahead extension discussed earlier this session: a
     pairwise boundary decision between position `t` and `t+1` is fully
     resolved the INSTANT byte `t+1` is sampled — no lookahead buffering
     at all, unlike the windowed multi-candidate form (which genuinely
     needs `W` future bytes that don't exist yet during autoregressive
     decode). So `generate_kv_cache`'s existing incremental-append pattern
     (already built for the fixed-`K` "append every `K` bytes" case)
     generalizes almost directly: right after sampling byte `t+1`, check
     the boundary condition once; if true, quantize and append EXACTLY
     ONE new KV entry (tagged with raw position `t+1` for its own future
     RoPE fusion reads) to level `i+1`'s cache — a content-triggered
     append instead of a counter-triggered one, same one-entry-per-event
     shape, no new buffering machinery. This "simpler at inference" only
     holds for the plain pairwise mechanism, though — the windowed/
     dilated generalization (session, several turns back) reintroduces a
     real bounded-delay buffer at generation time, a concrete reason to
     implement the plain pairwise form FIRST (point 3's own deferred note
     already recommended this on cost grounds; this is a second,
     independent reason on generation-simplicity grounds).

**Static-shape realization — fixed-capacity top-k, not dynamic
`cumsum`/`n_chunks.max()`.** Point 2's `cumsum(is_boundary)` +
scatter/argsort-gather is correct but produces a genuinely
DATA-DEPENDENT chunk count per row — bad for `torch.compile` (see
[docs/torch_compile.md](../docs/torch_compile.md)'s own MPS findings) and,
worse, unsafe: an unconstrained boundary detector can degenerate to
EITHER extreme — always-boundary (`is_boundary` all-ones, `K` collapses
to stride 1, `n_chunks = L`, zero compression, level `i+1` costs as much
as level `i`, defeating the entire hierarchy) or never-boundary
(`n_chunks` collapses toward 1, one giant chunk, level `i+1` loses
resolution entirely) — the two symmetric MoE-style collapse failure
modes, and the FIRST one is also a genuine memory blowup risk since nothing
bounds `n_chunks.max()` across a batch. Fix, mirroring how real capacity-
based MoE systems (Switch Transformer, GShard) handle exactly this,
applied here as a STATIC allocation, not a safety-valve fallback:

```python
C = max_chunks_allowed[level]        # Python int, config-fixed = ceil(L // K_min) — the ONLY
                                      # place "how much compression, worst case" is decided
scores = self.boundary_head(h)                              # [B, L], continuous
topk_scores, topk_idx = torch.topk(scores, k=C, dim=1)       # [B, C] each — ALWAYS exactly C,
                                                              # shape is a Python int, never data-dependent
sort_idx = topk_idx.sort(dim=1).values                       # restore left-to-right (causal) order
                                                              # among the C selected positions
h_selected = torch.gather(h, 1, sort_idx.unsqueeze(-1).expand(-1, -1, D))   # [B, C, D], STATIC
valid = torch.gather(scores, 1, sort_idx) > self.boundary_threshold        # [B, C] bool — genuinely
                                                              # a decided boundary, vs. "only present
                                                              # because top-k needed C slots filled"
pre_q = self.code_pre(h_selected)
c_i = bsq_quantize(pre_q, dq)         # [B, C, dq] — EVERY level-above-0 tensor is this shape,
                                       # always, regardless of content — `valid` is what actually
                                       # varies, carried downstream exactly like `pad_mask` already
                                       # is in the tiling section above
```

This directly answers "efficient mechanism if it degenerates to stride 1
always": the always-boundary collapse can no longer blow up `n_chunks`
past `C` — `topk` silently keeps only the `C` most-confident positions
and drops the rest, the same "capacity overflow → drop" policy real MoE
routers use under saturation, not a new mechanism invented for this file.
The never-boundary collapse is equally safe: `valid` simply comes back
mostly/all `False`, `c_i`'s shape is UNCHANGED (still `[B, C, dq]`), and
everything downstream (loss reduction, `jagged_causal_mask_and_positions`,
RoPE position tagging via `sort_idx` instead of an assumed `(b+1)*K-1`)
already has to consult `valid`/mask regardless of level — "pad if [fewer
real boundaries than] max len is reached" is exactly what `valid=False`
already expresses, no separate dynamic-vs-static code path needed. This
is the same "masked-but-computed" convention this file's own Axis 4
sketch (in `hierarchical_fusion_designs.md`) already established for
depth-routing — reused here for boundary-routing instead, for the same
reason (`torch.compile`-friendly, static shape first, real gather/scatter
compute savings deferred as a strictly harder second step).

**Soft constraint, min AND max, layered on top of the hard `C` cap —
different job, not a redundant second mechanism.** The static cap above
only guarantees SHAPE safety (bounded worst case, compile-friendly); it
says nothing about whether the model actually LEARNS a useful compression
ratio — nothing stops training from drifting toward "always saturate `C`"
(most spans dropped, degrades to a fixed stride-`L/C` grid, exactly what
this design was meant to escape) or "always near-zero real boundaries"
(most slots wasted on `valid=False` padding, so `C` is paid for in
compute/memory but rarely used) as OTHER training-dynamics failure modes,
distinct from the shape-blowup one the cap alone fixes. Reuse
`hierarchical_fusion_designs.md`'s own two-sided ratio-loss sketch
(Axis 4's "ratio/budget regularization" section) unchanged in mechanism,
just retargeted from a single point rate to a BAND: pick `r_min =
1/K_max`, `r_max = 1/K_min` (average stride bounded both directions, not
just one), and either fix both as plain hyperparameters first (this
file's own "confirm the loss converges to a static target before adding
a controller" ordering) or drive them from measured `val_bpb`/compute
cost via the same dual-ascent controller already sketched there. The cap
`C` and the ratio band are deliberately NOT the same knob: `C` is a hard,
static, worst-case shape bound chosen once at config time; `r_min`/`r_max`
are soft, learned-toward targets that keep TYPICAL behavior well inside
that bound so the `valid=False`-heavy (wasted compute) and
capacity-overflow (dropped information) regimes both stay rare in
practice, not just impossible in the worst case.

---

**Deliberately deferred, not part of this sketch**: an actual
`ratio_loss`/budget control (`hierarchical_fusion_designs.md`'s own Axis
4 sketch already works this out — targeting a mean chunk length/compute
budget rather than a raw fixed threshold) is the natural next layer once
this lands, but isn't needed to state the core mechanism change above.
Also deferred: which scoring form (plain pairwise vs. windowed/dilated
multi-lag) to start with — the plain pairwise `K=2` case is the cheaper
first implementation per this session's own "prefer simple things first"
precedent, with the windowed form as a documented follow-up, not a
prerequisite.
