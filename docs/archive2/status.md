...
| model | final train bpb | best val bpb |
|---|---|---|
| v4.4 (`k4_1_l1`) | 0.3495 | 3.7404 |
| v4.5 (`k4_1_l2`) | 0.1397 | 3.9460 |
| bytelm (`l1`) | 0.0212 | 3.9495 |
| bytelm (`l2`) | 0.0072 | 3.9552 |
...
**Results (same offset-5850 prompt, all `step=1000`, `last.pt`):**

| config | train bpb | EXACT cond | cond generation |
|---|---|---|---|
| v4.4.1 `k1` | 0.1618 | False | garbled |
| v4.4.1 `k1_k1` | 0.1405 | **True** | verbatim |
| v4.4.1 `k4_1` | 0.0816 | False | `mic institutions, anarchists advocate social relaios been [[na i` (near-exact, drifts late) |
| v4.4.1 `nogumbel_k1` | 0.1118 | False | garbled |
| v4.4.1 `nogumbel_k1_k1` | 0.0845 | **True** | verbatim |
| v4.4.1 `nogumbel_k4_1` | 0.0848 | **True** | verbatim |
| v4.5.1 `k1` | 0.1546 | False | garbled |
| v4.5.1 `k1_k1` | 0.1336 | False | garbled |
| v4.5.1 `k4_1` | 0.1147 | False | garbled |
| v4.5.1 `nogumbel_k1` | 0.1135 | False | garbled |
| v4.5.1 `nogumbel_k1_k1` | 0.1020 | False | garbled |
| v4.5.1 `nogumbel_k4_1` | 0.1038 | False | garbled |

**v4.4.1 gets exact verbatim reproduction on 3/6 configs** (`k1_k1`, `nogumbel_k1_k1`,
`nogumbel_k4_1`) -- notably `k1_k1` achieves this WITH gumbel noise on, which the earlier v4.4
grid (pre-LM-continuation-fix) never managed at this prompt; `k4_1` (gumbel on) misses exact match
but tracks the ground truth for the first ~50 bytes before drifting, still far more coherent than
anything in the pre-fix v4.4/v4.5 grids at K>1. **v4.5.1 gets 0/6 exact matches**, every config
garbled -- same pattern as every prior v4.5/v4.5.1-style cross-attention-flavored decode this
session: the LM-continuation fix helps v4.4.1's packed-self-attention decode dramatically but does
nothing for v4.5.1's decode quality, reinforcing the standing finding that v4.5's problem is
architectural (staged cross-attention exposure bias), not the conditioning-alignment bug that was
just fixed.

**Vs. baselines at this same prompt**: bytelm `l1`/`l2` were previously found near-fully memorized
at this scale (train bpb 0.02/0.007, see "final-step TRAIN bpb" table above) but were not
re-surveyed at this exact offset in this session; the earlier v4.4 no-share grid's single best
result (`nosharegrid_nogumbel_k4single`, pre-LM-continuation-fix) got exact match on 3/4 DIFFERENT
sampled train prompts (§ "Full same-prompt generation survey"). v4.4.1's 3/6 exact-match rate at
THIS harder single prompt, achieved with substantially higher train bpb (0.08-0.14 vs. bytelm's
0.007-0.02) and including a gumbel-on config, is the strongest qcute_refine-lineage result yet for
how cheaply (in train-bpb terms) exact reconstruction is reached -- consistent with the
LM-continuation fix (§ above) being a genuine, not cosmetic, improvement for v4.4-style decode.

## `decode_code_ste` dead-gradient finding, and the `k1_l1` STE experiment

**Finding**: with `decode_code_ste=False` (the default used by every grid config above) and
`n_levels=1`, the code-producing `code_head` (`_classify`, independent `nn.Linear(D,V)` since
`code_head_tied` defaults `False`) receives **zero gradient, ever**. Decode's conditioning input
is built as `self.decode_lms[i].embed(source_c.argmax(-1))` — `argmax` blocks gradient from
reaching `source_c`, hence `code_head`, and for `n_levels=1` nothing else differentiably consumes
`source_c` (no coarser level above to read it via the always-differentiable STE path). Verified
directly in both `v4.4.1` and `v4.5.1`: `code_head.weight.grad is None` after `.backward()`.

Despite this, `n_levels=1` configs still converge and sometimes hit exact reconstruction (§ above,
`v4.4.1 k1_k1`/`nogumbel_k1_k1`/`nogumbel_k4_1`). Why: `self.embed` (the `nn.Embedding` table) is a
**separate** parameter from `code_head`, and `embed(idx)` is an ordinary differentiable-in-weight
lookup — decode's NTP loss trains `embed.weight` normally regardless of STE. So `code_head` acts as
a frozen-at-init, effectively-random hash from (a moving, separately-trained) pooled hidden state to
a code id; `embed.weight` then learns to memorize "what follows code id k" for whichever ids that
frozen hash happens to emit. This works at overfit10k scale (vocab ≫ #distinct blocks, collisions
rare) but is closer to VQ-VAE's codebook-content channel operating *without* its usual STE-trained
routing/encoder side, not a real discrete-bottleneck bottleneck being learned — see the direct
answer to the user's question in-chat for the full contrast with VQ-VAE's stop-gradient trick.

**Experiment**: `decode_code_ste=True` on `Ks=(1,)`, `n_layers=1` (not 2 — a params/depth-reduced
cell relative to the grid above, to keep smoke-test/iteration fast), gumbel on, `share_level_weights=
False`, otherwise standard overfit10k (`configs/qcute_refine_v4_4_1_ste_k1_l1.py`,
`qcute_refine_v4_5_1_ste_k1_l1.py`). Paired with `decode_ntp_weight=0.0` twins
(`*_ste_uncondonly_k1_l1.py`) that zero out decode's loss entirely — isolates whether STE's now-live
gradient into `code_head` (only possible when `decode_ntp_weight>0`, since STE routes decode's own
loss backward) does anything for the encode LM's own unconditional generation quality.

**Results (same offset-5850 prompt, `step=1000`, `last.pt`):**

| config | train bpb | best_val_bpb | EXACT cond | cond generation | uncond generation |
|---|---|---|---|---|---|
| v4.4.1 `ste_k1_l1` | 0.088 | 3.7565 | False | `tttt lllllls lllyuuuccittttttette t t  ttr  tt      tttttttttttt` (repetition collapse) | `menng thectcriastiiadiallkedialy wath,   evou&quot;rstheggggggia` |
| v4.5.1 `ste_k1_l1` | 0.095 | 3.7126 | False | ` nnntoto t ttttttttteeeeeee s k srrrngnnnnaaaattttarrrrttiiiiiii` (repetition collapse) | `mte</y fre</y>\n      tevefethee.llim[[[veventiofreve  /yyy >\n   ` |
| v4.4.1 `ste_uncondonly_k1_l1` (decode_ntp_weight=0) | n/a | n/a | False | `NNBBB\x80\x80\x80\x80\xba\xbaNNNN...` (decode never trained, expected) | `miond sss teeed weeeeeeeed [[[Tan p556aooo55<i, rrconamey rciana` |
| v4.5.1 `ste_uncondonly_k1_l1` (decode_ntp_weight=0) | n/a | n/a | False | `\xe8\xe8\xe8\xe8*\xab\xb4\xb4...` (decode never trained, expected) | `me [[[[hestoris]]s has|sssthh rritis hisagalllsoccstan iss in [[` |

(`ste_uncondonly` rows' `cond generation` is meaningless by design — `decode_ntp_weight=0` means
decode's weights are never trained, so `generate_no_cache` output there is just untrained-decode
noise, not a quality signal.)

**Verdict: `decode_code_ste=True` did not help, and looks actively worse for cond generation.**
Train bpb dropped to 0.088-0.095 (vs. the `n_layers=2`, `decode_code_ste=False` `k1` baselines'
0.1618/0.1546 — note this comparison is confounded by `n_layers` 1 vs 2, not STE alone), but
`val_bpb` stayed stuck around 3.71-3.76 with `val_byte_acc` plateauing near 38-39% (train byte_acc
reached ~98%) — a much wider train/val gap than anything in the `decode_code_ste=False` grid, i.e.
worse overfitting, not better generalization. More tellingly, greedy `cond_full` generation
**collapsed into single-character repetition** (`tttttt...`, `nnnn...`) for both `ste_k1_l1` runs —
strictly worse than the `decode_code_ste=False` `k1` baselines' generation, which was "garbled" but
not degenerate repetition. STE gradient flowing into `code_head` appears to let the model fit the
training codes/bytes extremely tightly under teacher forcing while making autoregressive rollout
*more* brittle (exposure bias), not less.

**Uncond quality comparison (`ste_k1_l1` vs `ste_uncondonly_k1_l1`, isolating whether STE's
decode→encode gradient helps encode's own unconditional generation)**: no clear win either way.
`v4.4.1`: both produce similarly noisy word-fragment text (`menng thect...` vs `miond sss teeed...`)
with no obvious quality gap. `v4.5.1`: `ste_uncondonly`'s uncond output (`me [[[[hestoris]]s
has|sssthh...`) arguably has slightly more wiki-markup-like structure (`[[`, `]]`, `|`) than
`ste_k1_l1`'s (`mte</y fre</y>...`), i.e. if anything the run WITHOUT the STE-derived gradient looks
marginally better, not worse. So the hypothesized benefit — decode's loss finally training
`code_head` via STE, which could in principle shape better codes and thus better uncond quality —
is not visible at this scale/step budget; the dominant effect of turning on `decode_code_ste=True`
was the cond-generation collapse above, not an uncond improvement.

**Conclusion**: keep `decode_code_ste=False` as the default for now. The dead-gradient issue is real
or `n_levels=1` code_head-in-particular, but `=True` trades it for worse exposure-bias behavior at
this tiny scale, at least at `n_layers=1`. Revisit at larger scale/longer training budgets, and
consider whether the fix should be scoped differently (e.g. only route STE gradient into `code_head`
without also changing decode's conditioning-input differentiability, or add an explicit commitment
loss VQ-VAE-style rather than relying on decode's NTP loss alone).

**Follow-up: gumbel noise on/off doesn't rescue it either.** Reran `ste_k1_l1` with
`use_gumbel_noise=False` (`configs/qcute_refine_v4_{4_1,5_1}_ste_nogumbel_k1_l1.py`, otherwise
identical):

| config | train bpb | best_val_bpb | cond generation | uncond generation |
|---|---|---|---|---|
| v4.4.1 `ste_nogumbel_k1_l1` | 0.104 | 4.0623 | `oooooooo            <<<<<<<<<<i<i//nnntntaaaaddddle    <<<<<<i<i` | `mis  and conns itnannckinnnnssss ussocrgikinnnnntitionnng ry ee ` |
| v4.5.1 `ste_nogumbel_k1_l1` | 0.082 | 4.0007 | `uuuunnnnnnnaaaanaaaarrrrrriiiiuneunnnnaaaaannanarrrrrrrciiaaaiaa` | `mauttthe boly revvery atthe ccal  [[[[<rathory lsssss de revey R` |

Still degenerate repetition-collapse in cond generation for both (vowel/character clustering,
same failure mode as gumbel-on), and `best_val_bpb` is actually worse than the gumbel-on STE runs
(4.00-4.06 vs 3.71-3.76). So the collapse traces to `decode_code_ste=True` itself, not to gumbel
noise's interaction with it -- confirms the earlier conclusion holds independent of gumbel on/off.

**Addendum**: the STE gradient path doesn't stop at `code_head`. `code_embeds0 = source_c0 @
stage_lm0.embed.weight` (v4.5.1) multiplies `encode_lms[i]`'s own `source_c0` (not detached) by the
DECODE module's embed table (a separate `LevelLM` instance under `share_level_weights=False`) —
gradient flows back through `source_c0` into `encode_lms[i].code_head` **and through it into
`encode_lms[i].blocks`**, the same trunk `generate_encode_only`/uncond generation reads. So
`decode_code_ste=True` doesn't just un-freeze `code_head`, it couples decode's harder
discrete-bottleneck loss into the encode trunk's own representations. This is consistent with
`ste_uncondonly` (decode loss fully zeroed, no such coupling) producing more structured uncond text
than `ste_k1_l1` (coupling live) — evidence the coupling is actively distorting the shared trunk at
this scale, not just failing to help.
...
## True incremental KV cache design notes (2026-08-17, in progress)

Both `qcute_v5.py`/`qcute_v5_concat.py`'s existing `generate_kv_cache` is FIFO-window
*re-truncation* (recomputes the full windowed forward every step), not a real per-layer K/V tensor
cache. Investigated feasibility of a true cache: both files already precompute buffer/attention
addressing as a pure function of shape (cached across calls), and RoPE positions are per-level
LOCAL (reset to 0 every call, in that level's own granularity units — bytes for level 0, codes for
level 1, etc.), never a shared cross-level clock. Since RoPE attention depends only on relative
position offset, switching each level's own local counter to absolute/monotonically-increasing
(instead of resetting per call) is a free correctness win enabling real caching, though multi-rate
code-append cadence (per-level, per-`K`) and (for `qcute_v5.py`) the qfb boundary-query pass's own
separate append stream are real remaining engineering work. **Implemented and verified for
n_levels==1** in both files (exact token-for-token match vs. `generate_no_cache`, 4.5-5x wall-clock
speedup vs. the existing FIFO `generate_kv_cache`). Full design + speedup numbers:
[kv_cache_design.md](kv_cache_design.md).

## Bug found and fixed: qcute_v5_concat.py's chunked windowed decode didn't match its own dense reference

While validating the new KV cache, found that **`qcute_v5_concat.py`'s `_merged_decode_forward`
computed a different result via its chunked/banded attention path than via its own dense-masked
fallback, for the identical window** — confirmed on the actual trained shape (`Ks=(4,1)`,
`attn_window=(16,64)`). Root cause: `_merged_layout`'s `n_prev_chunks` undercounted how many prior
buffer-index chunks are needed once codes are interleaved into the byte buffer (multiple buffer
entries can share a `true_pos` unit, especially at small `K`), so chunked queries silently saw a
narrower effective window than configured. **Fixed** (density-aware chunk-lookback count); verified
against the dense reference and via `check_gen_consistency` across `Ks in
{(1,),(2,),(4,),(2,2),(4,2),(2,1,1)}` with no regression. An initial version of this report also
flagged `qcute_v5.py`'s `cross_attn_stage` as affected — that was a false positive from an invalid
comparison (see the doc's "Correction" section); `qcute_v5.py` was never affected and needed no fix.
All training was stopped when this was found; the `qcute_v5_2*`/`qcute_v5_3`/`qcute_v5_concat_2*`/
`qcute_v5_concat_3` batch's logs (trained under the pre-fix code) were deleted and the batch was
requeued with the fix in place.

**Confirmed the bug affected training, not just generation**: loaded `checkpoints/qcute_v5_concat_1/best.pt`
and computed real-data val bpb on the same enwik8 validation slice via `git stash`-toggling pre-fix vs.
post-fix code — `2.6700` (pre-fix) vs. `2.6604` (post-fix), a genuine loss-computation difference, not
just a generation-time artifact. `qcute_v5_concat_1` itself was trained/evaluated under the pre-fix
code and is thus suspect, but its checkpoint/logs have not been deleted or rerun — it predates this
session's requeue batch and isn't part of it.

Since the fix changes the loss landscape, the full 10-job batch (`qcute_v5_2_bsq8`, `qcute_v5_2_bsq16`,
`qcute_v5_2_gumbel`, `qcute_v5_2`, `qcute_v5_3`, and their `qcute_v5_concat_2*`/`qcute_v5_concat_3`
counterparts) was killed a second time, all logs/checkpoints/`/tmp` files cleared, and relaunched fully
fresh from step 0 on the fixed code (confirmed clean restart: first job progressing normally). Full
repro, root cause, fix, and impact: [chunked_decode_window_bug.md](chunked_decode_window_bug.md).
