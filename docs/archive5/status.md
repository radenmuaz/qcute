# qcute v1 status (archived 2026-08-23)

Archived in full — superseded by a fresh `docs/status.md`, pruned for size once `qcute_zero`
became a second fully-fledged active lineage alongside `qcute_v1` and this file grew past 1300
lines. Everything below is preserved verbatim for historical reference (StackDecoderV1's
interleaved-seed-token design, the v1 naming fix, hard-exclusion, `StackDecoderLocal`'s
generation bug, the `kv_lm_mode`/`decode_scope`/`byte_head` weight-reuse work, and the
`own_code_min_lag`/`bpb_valid`/`generate_free_rollout` work that immediately preceded this
prune). For anything current, see the live `docs/status.md`; for the underlying architecture
narrative (not results), see `CLAUDE.md`'s Architecture section and `docs/maths.md` (the
formal bpb-validity/paradigm-comparison writeup this session produced). Older still: full
v5-era history at [docs/archive4/status.md](archive4/status.md),
