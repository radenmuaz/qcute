"""Full-attention-encoder variant of image_gen_cifar, standalone codebase (no imports
from run_causalattn.py or qcute_lagcodec -- every primitive rewritten here).

Difference from run_causalattn.py: ONLY the encoder's attention type changes, from
causal to full (bidirectional). Same 3-level hierarchy, same strides=(2,4,4), same
per-row code shapes -- each level runs FULL (non-causal) self-attention independently
PER ROW (rows folded into the batch dim, never attending across rows -- this is what
keeps row r's code depending only on row r's own pixels, never future rows, which the
decoder's lag-1-row scheme still requires for a valid chain-rule NLL). Consequently
there is NO NTP head and no encoder-side generative prior (sample_codes()) at all: a
level's code is not something ever autoregressively generated in its own right, it's
only ever context handed to the decoder -- and the decoder's own real row(r)->row(r+1)
prediction (teacher-forced on real scanlines during training) already IS the model's
next-token-prediction mechanism. A second code-level NTP objective would be redundant.

Decoder is unchanged in kind from run_causalattn.py: the same causal, lag-1-row,
32-macro-step, GQA, column-batched (SISO/MIMO/grouped via col_group_size) decoder-only
transformer, bootstrapped by the same learned BOS vectors (bos_l2/bos_l1/bos_l0) for
row 0 -- chosen over a "trainable pixel scanline" alternative because every AR model
needs *some* placeholder for "nothing precedes the first step" (that's not a special
case, it's universal), and BOS sits directly in decoder-conditioning space (a short
gradient path) rather than forcing a bootstrap signal through the whole encoder stack
for no architectural benefit.

Generation is therefore ALWAYS a genuinely interleaved per-row loop (never the
run_causalattn.py two-phase "sample all codes, then decode" split, since there is no
code-level prior to front-load): encode row r (real prompt bytes, or the model's own
just-decoded row r) -> that row's codes condition the decoder's row r+1 prediction ->
repeat. A prompt (real given rows) and the model's own prior output are handled
identically once realized -- both just get encoded the same way. Prompted rows also
populate the decoder's own R/G/B slots with the same real bytes (not just the encoder),
matching exactly how every row is treated during training (teacher forcing).
"""
import argparse
import json
import math
...
def codebook_utilization(idx: torch.Tensor, vocab: int) -> torch.Tensor:
    """idx: (...,pq_chunks) long code ids. Logged-only diagnostic (no gradient) -- per-chunk
    usage-perplexity normalized to [0,1], averaged over chunks."""
...
# ---------------------------------------------------------------------------
# Encoder: 3-level FULL-ATTENTION hierarchy, no NTP, batched over ROWS
# ---------------------------------------------------------------------------
...
class FullAttnEncoderLevel(nn.Module):
    ...
    def encode(self, x_embed: torch.Tensor) -> tuple:
        h = self.run(x_embed)
        pooled = self.pool(h)
        logits = self.code_head(pooled).view(*pooled.shape[:-1], self.cfg.pq_chunks, self.cfg.code_vocab)
        return quantize_hard(logits)  # (...,pq_chunks,V), (...,pq_chunks)
...
class ImageEncoderFullAttn(nn.Module):
    ...
    def encode_rows(self, r_rows: torch.Tensor, g_rows: torch.Tensor, b_rows: torch.Tensor) -> dict:
        ...
        code2_soft, code2_idx = self.level2.encode(x2)  # (M,1,pq,V), (M,1,pq)
        return dict(code0_soft=code0_soft, code0_idx=code0_idx, code1_soft=code1_soft, code1_idx=code1_idx,
                    code2_soft=code2_soft.squeeze(1), code2_idx=code2_idx.squeeze(1))
    ...
    def forward(self, r: torch.Tensor, g: torch.Tensor, b: torch.Tensor) -> dict:
        ...
        def unfold_L(x, L_per_row):
            return x.reshape(B, img * L_per_row, *x.shape[2:])

        code0_soft = unfold_L(out["code0_soft"], out["code0_soft"].shape[1])
        code0_idx = unfold_L(out["code0_idx"], out["code0_idx"].shape[1])
        code1_soft = unfold_L(out["code1_soft"], out["code1_soft"].shape[1])
        code1_idx = unfold_L(out["code1_idx"], out["code1_idx"].shape[1])
        code2_soft = out["code2_soft"].reshape(B, img, *out["code2_soft"].shape[1:])  # (B,img,pq,V)
        code2_idx = out["code2_idx"].reshape(B, img, *out["code2_idx"].shape[1:])     # (B,img,pq)
        ...
        return dict(code0_soft=code0_soft, code1_soft=code1_soft, code2_soft=code2_soft, **util)
...
class Decoder(nn.Module):
    ...
    def row_cond_from_codes(self, code2_row, code1_row, code0_row, y_embed: torch.Tensor = None) -> tuple:
        """code2_row: (B,pq[,V]); code1_row: (B,n_l1,pq[,V]); code0_row: (B,n_l0,pq[,V])
        -- ONE row's own codes (real or just-realized) -> (l2e,l1e,l0e) each (B*img,D),
        ready to condition the NEXT row's decode. Same per-column broadcast/grouping/
        col_mix as _per_column_cond, just for a single row instead of all of them at
        once -- used by the interleaved generation loop, which computes each row's
        conditioning on the fly rather than having every row's codes precomputed."""
        cfg = self.cfg
        img = cfg.img_size
        D = cfg.d_model
        B = code2_row.shape[0]
        ...
        l2e = code_embed(code2_row, self.dec_l2_embed)  # (B,D)
        l1e = code_embed(code1_row, self.dec_l1_embed)  # (B,n_l1,D)
        l0e = code_embed(code0_row, self.dec_l0_embed)  # (B,n_l0,D)
        ...
        for row in range(img):
            ...
            if row < img - 1:  # no need to encode the last row -- nothing left to condition
                ...
                l2e_prev, l1e_prev, l0e_prev = dec.row_cond_from_codes(
                    enc_out["code2_idx"], enc_out["code1_idx"], enc_out["code0_idx"], y_embed)
...
def main():
    ...
    p.add_argument("--run_name", type=str, default="cifar_fullattn_minimal")
        ...
        for r, g, b, y in pbar:
            ...
            if step % args.log_every == 0:
                logger(f"epoch={epoch} step={step} bpb={out['bpb'].item():.4f} acc={out['acc'].item():.4f} "
                       f"util(l0/l1/l2)={out['util0'].item():.2f}/{out['util1'].item():.2f}/{out['util2'].item():.2f}",
