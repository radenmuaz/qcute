...
n_layers=8, n_heads=4, mlp_mult=2, head_dim=64, context=1024, ~4.3M params at mtp_heads=1) on
the full enwik8 corpus (datasets/enwik8.gz, 100,000,000 bytes).
...
context(1024) + mtp_heads(1) = 1025, steps_per_epoch = 90,000,000 / (batch_size(128) * 1025)
=~ 686.0 -> steps = round(686.0 * 3) = 2058. warmup_steps=34 =~ 5% of one epoch, then constant
LR (cosine_decay=False), same schedule convention as the tiny_full_enwik8 config.
