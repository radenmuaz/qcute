...
k32_narrow_attn_id4_hfull.py, ONE change: `bit_downsample_h=True` instead
...
because of downsample h." Direct A/B pair with `attn_id4_hfull.py` — same
downsample ratio (4), same everything else, isolating whether
downsampling h itself (not just the embed/attention machinery) causes a
real quality loss.

Everything else identical to attn_id4_hfull.py: Ks=(32,32), dq=8,
...
bit_per_position_head=True, code_embed_mode="pq_table", steps=4000.
    ...
    uv run python -m qcute.qcute_refine_v4_2 --config configs/qcute_refine_v4_2_k32_narrow_attn_id4_hds.py

    # plot after training:
    uv run python scripts/plot_run.py logs/qcute_refine_v4_2_k32_narrow_attn_id4_hds
"""
