...
byte+MTP baseline on the tiny corpus.

Result when this was run: best val_bpb 2.52 at step 1300, then mild
overfitting drift by step 2000 (see docs/status.md's comparison table and
logs/bytelm_xs_mtp4_converged/bpb.png).
    ...
    # plot after training:
    uv run python scripts/plot_run.py logs/bytelm_xs_mtp4_converged

The config's filename stem (bytelm_xs_mtp4_converged) becomes the run_name
