"""QueryClassifierHead variant of query_hourglass_tiny_1.py -- SIMPLE two-tier width instead of a
meticulous per-stage doubling pyramid: first half of the chain (4 stages) stays thin (d_model=32),
second half (4 stages) is fat (d_model=256). One transition, not a gradual ramp.

Same stride=4/8-stage shape validated for connectivity this session. cum_stride=4^8=65536
(43.5% of the 150528-length image) -- see module docstring in query_hourglass_tiny_1.py for why
QueryClassifierHead doesn't need full coverage (async query aggregation, not a causal last-token
dependency chain); this lands a bit above the 10-40% heuristic ceiling, acceptable for a first cut
of the two-tier variant since it's still far short of the old causal-decoder's full-coverage need.

    uv run python summformer_jax/image_classification/train.py --config configs/query_hourglass_tiny_2.py --shard-dir /dev/shm/imagenet_raw
"""
