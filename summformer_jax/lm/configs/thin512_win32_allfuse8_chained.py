"""Window-ablation sibling of thin512_win16_allfuse8_chained.py -- WINDOW=32 (generous, well above
the confirmed receptive-field minimum ~7-10), the most expensive point in the window sweep
(8/12/16/32) that finds the pareto-optimal self-attn window (effectively the KV cache size) for
this stride=2/8-stage/L=1024 shape. Cross-attn window matches self-attn window (not cumprod/-1
auto-derive), same rationale as the other sweep siblings. use_sink left at default True (see
thin512_win8_allfuse8_chained.py's docstring for why use_sink=False is unsafe for this causal-AR-
decoder-with-cross-attn topology).

    uv run python summformer_jax/lm/train.py --config configs/thin512_win32_allfuse8_chained.py --data-dir data/fineweb-edu-10B
"""
