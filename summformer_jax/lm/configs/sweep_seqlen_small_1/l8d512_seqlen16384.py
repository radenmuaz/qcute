...
N_LAYERS = 8
N_STAGES = 8
WINDOW = CROSS_WINDOW = 256
STRIDE = 2
SEQUENCE_LENGTH = 16384
WEIGHT_TIE = False
sequence_length = SEQUENCE_LENGTH  # lowercase alias so train.py's config loader picks it up (recognized keys are argparse dest names, lowercase)

batch_size = 8  # 16 * 16384 * 4 devices > total_batch_size=524288; 8 is the largest that divides evenly
...
def build_summformer(rngs: nnx.Rngs) -> ARHead:
    ...
    encoder = Encoder(StackConfig(n_layers=0, d_model=D_MODEL, n_heads=N_HEADS, pos_method=pos_method,
                                   compute_dtype=jnp.bfloat16, param_dtype=jnp.float32, use_remat=True),
