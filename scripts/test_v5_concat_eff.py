"""CPU-only correctness check for qcute_v5_concat_eff.py against the original qcute_v5_concat.py:
(1) chunked_windowed_attention vs a brute-force dense reference, (2) full RefineLM forward
equivalence with copied weights (orig vs eff), (3) check_gen_consistency on the eff model.

    uv run python scripts/test_v5_concat_eff.py
"""
import torch
import torch.nn.functional as F

from qcute import qcute_v5_concat as orig
from qcute import qcute_v5_concat_eff as eff
...
def build_pair(Ks, context_len, attn_window, n_layers=2, d_model=32, n_heads=2):
    cfg_kwargs = dict(
        ...
        attn_window=attn_window, decode_self_only_aux=False, use_gumbel_noise=False,
        share_level_weights=True, decode_code_ste=True,
    )
    cfg_o = orig.Config(**cfg_kwargs)
...
def test_forward_equivalence():
    ...
    cases = [
        ...
        ("Ks=(2,2) multi-track banded + single-track chunked", (2, 2), 64, (8, 8)),
        ("Ks=(4,2) multi-track banded + single-track chunked", (4, 2), 64, (4, 4)),
        ("Ks=(2,1,1) 3-level: multi-track + single-track chunked", (2, 1, 1), 64, (4, 4, 4)),
    ]
    torch.manual_seed(1)
    for name, Ks, context_len, window in cases:
        ...
        max_metric_diff = max((metrics_o[k] - metrics_e[k]).abs().item() for k in metrics_o)
