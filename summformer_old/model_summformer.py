...
def sdpa_with_sink(q: jnp.ndarray, k: jnp.ndarray, v: jnp.ndarray, attn_mask: jnp.ndarray,
    ...
    """Zero-value/zero-key sink (on by default) -- every query row keeps >=1 valid key, so a
    sink-only row (nothing else visible) is a provably clean zero instead of a NaN/uniform-softmax
    edge case. `use_sink=False` is a pure runtime toggle (the sink carries no learned parameters,
    just a per-call zero row) -- safe to flip without touching any trained checkpoint's weights;
    added to isolate the sink's own cost/effect for benchmarking (see
    Config.zero_kv_sink / docs/status_tpu.md)."""
...
class FuseStage(nnx.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, mlp_mult: int, n_layers: int, scale_layers: int, dtype, param_dtype, *,
                 rngs: nnx.Rngs, use_sink: bool = True):
...
class Config:
    ...
    zero_kv_sink: bool = True         # runtime attention toggle, no learned params -- safe to flip
                                       # without affecting any trained checkpoint's weights; added
                                       # to isolate the sink's own cost for benchmarking
...
class SummTransformer(nnx.Module):
    def __init__(self, cfg: Config, *, rngs: nnx.Rngs):
        ...
        n_lms = self.n_fuse + 1
        if cfg.share_lm:
            first = nnx.List([Block(D, cfg.n_heads, cfg.mlp_mult, scale_layers, dtype, param_dtype, rngs=rngs, use_sink=cfg.zero_kv_sink) for _ in range(cfg.n_layers)])
            ...
            self.lms = nnx.List(
                [nnx.List([Block(D, cfg.n_heads, cfg.mlp_mult, scale_layers, dtype, param_dtype, rngs=rngs, use_sink=cfg.zero_kv_sink) for _ in range(cfg.n_layers)])
        ...
        # weight_tie=True reuses embed's own matrix (see _head_weight) -- don't also allocate an
        # untied D*V kernel that would then sit dead, never read by forward or written by backward.
        self.head = (
        ...
        if cfg.share_fuse:
            first_fs = FuseStage(D, cfg.n_heads, cfg.mlp_mult, fuse_layers, scale_layers, dtype, param_dtype, rngs=rngs, use_sink=cfg.zero_kv_sink)
            self.fuse_stages = nnx.List([first_fs for _ in range(self.n_fuse)])
        else:
            self.fuse_stages = nnx.List(
                [FuseStage(D, cfg.n_heads, cfg.mlp_mult, fuse_layers, scale_layers, dtype, param_dtype, rngs=rngs, use_sink=cfg.zero_kv_sink) for _ in range(self.n_fuse)])
